...
title: "eBay:iPads:: sold classification:: dataclns"
author: "bdanalytics"
...
#stop(here"); sav_allobs_df <- glb_allobs_df; glb_allobs_df <- sav_allobs_df
tmp_feats_vctr <- names(glb_allobs_df)
tmp_feats_vctr <- tmp_feats_vctr[grepl("D\\.npnct[0-9][0-9]\\.log", tmp_feats_vctr)]
for (feat in tmp_feats_vctr)
    glb_allobs_df[, paste0(feat, ".zval")] <- scale(glb_allobs_df[, feat])[, 1]
shapiro.test(glb_allobs_df[, feat])$statistic
shapiro.test(glb_allobs_df[, paste0(feat, ".zval")])$statistic
myplot_scatter(glb_allobs_df, feat, paste0(feat, ".zval"))
...
indep_numeric_vars_vctr <- subset(glb_feats_df, !myNearZV &
                                            (exclude.as.feat != 1))[, "id"]
indep_numeric_vars_vctr <- indep_numeric_vars_vctr[!grepl("\\.fctr",
                                                         indep_numeric_vars_vctr)]
glb_feats_df$shapiro.test.W <- NA
glb_feats_df[glb_feats_df$id %in% indep_numeric_vars_vctr, 
             "shapiro.test.W"] <- sapply(indep_numeric_vars_vctr, 
    function(feat) shapiro.test(glb_allobs_df[, feat])$statistic)
print(orderBy(~-shapiro.test.W, subset(glb_feats_df[, c("id", "shapiro.test.W")],
                                       !is.na(shapiro.test.W))))
chk_feats_vctr <- orderBy(~shapiro.test.W, 
                    subset(glb_feats_df[, c("id", "shapiro.test.W")],
                        !is.na(shapiro.test.W) & (shapiro.test.W < 0.75)))[, "id"]
plt_feats_vctr <- c(".rnorm", chk_feats_vctr[1:min(5, length(chk_feats_vctr))])

# check distributions in entire data (incl. test) for outliers that might be 
#   distorting the distributions
plt_df <- melt(glb_allobs_df[, c(glb_id_var, plt_feats_vctr)], id.vars=glb_id_var)
...
#                         X, ""))
plt_df <- merge(plt_df, glb_feats_df[, c("id", "cor.y")], 
                        by.x="variable", by.y="id", all.y=FALSE)
print(ggplot(plt_df, aes(x=factor(0), y=value)) + 
          geom_violin(aes(fill=cor.y)) + 
          #geom_text(aes(label=X_label), color="slategrey", size=3) + 
          facet_wrap(~ variable, scales="free_y") + xlab(""))

print(myplot_box(glb_allobs_df, ".rnorm", glb_rsp_var))
print(myplot_box(glb_allobs_df, chk_feats_vctr[2], glb_rsp_var))
